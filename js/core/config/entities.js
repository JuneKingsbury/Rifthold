export const ENTITIES = {
    // === Animals ===
    deer:    { char: 'd', color: '#bb8855', hp: 40, speed: 0.22, category: 'animal', hostile: false, meatYield: 3, hideYield: 2, fleeRange: 5, spawnWeight: 15, lore: 'An offshoot of the common mule deer. These skittish creatures are found all throughout the Frontier.', tameable: true, tamed: { foodToTame: 4, courage: 0.3, petDamage: 2, roles: [{ type: 'pack', expeditionSpeedBonus: 0.10 }, { type: 'wander' }], effects: [] } },
    rabbit:  { char: 'r', color: '#ccaa88', hp: 10, speed: 0.3, category: 'animal', hostile: false, meatYield: 1, fleeRange: 4, spawnWeight: 12, lore: 'A long eared rodent found in the forests of the Frontier.', tameable: true, tamed: { foodToTame: 1, courage: 0.05, petDamage: 1, roles: [{ type: 'wander' }], effects: [{ type: 'mood_aura', scope: 'aura', radius: 3, moodBonus: 2 }] } },
    ...
    okapi:   { char: 'O', color: '#b3562e', hp: 100, speed: 0.34, category: 'animal', hostile: false, meatYield: 1, fleeRange: 4, spawnWeight: 11, lore: 'A rare and elusive creature found in the dense forests of the Frontier.', tameable: true, tamed: { foodToTame: 5, courage: 0.4, petDamage: 3, roles: [{ type: 'pack', expeditionSpeedBonus: 0.25 }, { type: 'wander' }], effects: [] } },
    tapir:   { char: 't', color: '#f2e6e6', hp: 60, speed: 0.11, category: 'animal', hostile: false, meatYield: 1, fleeRange: 4, spawnWeight: 11, lore: 'A large herbivore with a distinctive snout found in the swamps of the Frontier.', tameable: true, tamed: { foodToTame: 3, courage: 0.2, petDamage: 1, roles: [{ type: 'wander' }], effects: [{ type: 'mood_aura', scope: 'aura', radius: 4, moodBonus: 5 }] } },
    ...
    boar:    { char: 'b', color: '#7a5c44', hp: 70, speed: 0.18, category: 'animal', hostile: false, meatYield: 3, hideYield: 1, fleeRange: 5, spawnWeight: 10, lore: 'A stocky and ill-tempered wild pig found throughout the Frontier. Though it flees at first, it does not forget an intrusion.', tameable: true, tamed: { foodToTame: 5, dangerousTame: true, baseTameChance: 0.55, retaliationDamage: 10, courage: 0.8, petDamage: 6, roles: [{ type: 'war', beastHp: 65, beastDamage: 14 }], effects: [] } },
...
export const TAMED_ANIMALS = Object.fromEntries(
    Object.entries(ANIMALS).filter(([, a]) => a.tameable).map(([k, a]) => {
        ...
        if (t.courage !== undefined) flat.courage = t.courage;
        if (t.petDamage !== undefined) flat.petDamage = t.petDamage;
        return [k, flat];
    })
);
